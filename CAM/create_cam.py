...
def create_cam(config):
    ...
    test_loader, num_class = CAM.utils.get_testloader(config.dataset,
                                        config.dataset_path,
                                        config.img_size)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
   
    cnn = cls.ClassificationModel(model_name="").create_model()
    cnn = cnn.to(device)
    cnn.eval()
    cnn.load_state_dict(
        torch.load(config.model_name)
    )
    finalconv_name = '_blocks'
    ...
    cnn._conv_head.register_forward_hook(hook_feature)
    params = list(cnn.parameters())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path', type=str, default='/root/torchbox/dataset/data_image/croped_val')
    parser.add_argument('--model_path', type=str, default='./model')
    parser.add_argument('--model_name', type=str, default='/root/torchbox/saved/models/20200807-2326-model1_train_6600_physical_augmentation1250.pth')
